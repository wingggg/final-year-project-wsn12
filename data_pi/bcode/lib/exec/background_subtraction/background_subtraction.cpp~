...
// 	    uniquect[unique[conn[p0]]]++;
    ...
    delete []conn;
    delete []unique;
    //output->writePNG("labels.png");cout << "saved " << lab << endl;//getchar();
...
int main(int argc, char **argv){
   ...
   if(!strcmp(par->getString("input_type.char"),"fulls")){
     for(uint i=1;i<filenames.size();i+=3){
        int sel = processSet(par, filenames, i, bbs);	
	if(bbs.size()>0)textoutput <<  filenames[i] << " " << bbs[0].valid << " " << bbs[sel].sx << " " << bbs[sel].sy << " " <<  bbs[sel].ex-bbs[sel].sx << " " << bbs[sel].ey-bbs[sel].sy  << " " << bbs[sel].confidence   << " " << bbs[sel].coverage  <<  "\n";
	...
	bbs.clear();     
        ...
        //textoutput << 0 << endl;
	if(bbs.size()>0)if(bbs[0].valid)textoutput <<  filenames[1] << " " << 1 << " " << round(conf_factor*bbs[sel].confidence) << " " << bbs[sel].sx << " " << bbs[sel].sy << " " <<  bbs[sel].ex-bbs[sel].sx << " " << bbs[sel].ey-bbs[sel].sy <<endl;
   }
