...
*************************************************************************/
	...
	col_size = image->x();
        ...
        DARY* xm = new DARY(row_size,col_size,FLOAT1);
...
*************************************************************************/
	...
	col_size = image->x();
        ...
        DARY* ym = new DARY(row_size,col_size,FLOAT1);
...
{
  //1=    0.1013 0.2620 0 -0.2620 -0.1013
    int rows, cols, r, c;
    float **pix, **rpix, p1;
    ...
    for (c = 0; c < cols; c++) {
      /* Handle easiest case of pixels that do not overlap the boundary. */
      for (r = 1; r < rows - 2; r++) {
	rpix[r][c] = -0.1013 * pix[r-1][c] -
	  0.2620 * pix[r][c]  + 0.2620 * pix[r+1][c] +
	  0.1013 * pix[r+2][c];
      }
      /* For pixels near boundary, use value of boundary pixel. */
      for (r = 0; r < 1; r++) {
	p1 = (r < 1) ? pix[0][c] : pix[1][c];
	rpix[r][c] = -0.1013 * pix[0][c] - 0.2620  * p1 +
	   0.2620 * pix[r+1][c] + 0.1013 * pix[r+2][c];
      }
      for (r = rows - 2; r < rows; r++) {
	p1 = (r >= rows - 1) ? pix[rows-1][c] : pix[rows-2][c];
	rpix[r][c] = - 0.1013 * pix[r-1][c] -
	  0.2620 * pix[r][c]  + 0.2620 * p1 +
	   0.1013  * pix[rows-1][c];
      }
    }
}
...
void DerHConv4(DARY *image,  DARY *result)
{
  ...
  //1=  0.1013 0.2620 0 -0.2620 -0.1013
    ...
    for (r = 0; r < rows; r++) {
      ...
      for (c = 1; c < cols - 2; c++) {
	prc =  pixr + c;
	rpixr[c] = - 0.1013 * prc[-1] - 0.2620 * prc[0] +
	   + 0.2620 * prc[1] + 0.1013 * prc[2];
      }
      /* For pixels near boundary, use value of boundary pixel. */
      for (c = 0; c < 1; c++) {
	p1 = (c < 1) ? pix[r][0] : pix[r][1];
	prc =  pixr + c;
	rpixr[c] = - 0.1013 * pixr[0] - 0.2620 * p1 +
	  0.2620 * prc[1] + 0.1013 * prc[2];
      }
  ...
  //0.6 = 0.1664 0.6672 0.1664
	...
	rpix[r][c] =  0.1664 * prc[-1] + 0.6672 * prc[0] + 0.1664 * prc[1];
     ...
     void  smoothSqrt(DARY* image_in, DARY* smooth_image){
       DARY* image_tmp = new DARY(image_in->y(),image_in->x(),FLOAT1);
...
void  dX2b(char* image_in, char* dximage, uint w, uint h){
	uint s=w*h-2;
	int dx;
	for(uint x=1;x<s;x++){
		dx=image_in[x+1]-image_in[x-1];
		dx=(dx<-127)?-127:dx;
		dx=(dx>127)?127:dx;
		dximage[x]=(char)dx;
	}
...
void  dY2b(char* image_in, char* dyimage, uint w, uint h){
	uint s=(w-1)*h-1;
	int dy;
	for(uint x=w;x<s;x++){
		dy=image_in[x+w]-image_in[x-w];
		dy=(dy<-127)?-127:dy;
		dy=(dy>127)?127:dy;
		dyimage[x]=(char)dy;
	}
...
void  dX2(DARY* image_in, DARY* dximage){
	...
	for(uint y=0;y<image_in->y();y++){
      ...
      void dXX_YY3(DARY* image_in, DARY* dyimage){
        DARY *tmp = new DARY(image_in->y(),image_in->x(),FLOAT1);
	dXX3(image_in,tmp);
	...
	DerHConv6(image_in,image_tmp);
        //DerHConv6(image_in,smooth_image);
        VerConv5(image_tmp,smooth_image);
	delete image_tmp;
      }     
  ...
  for (uint i = 0; i < im->size(); i++){
	...
	delete []coefs;delete image_tmp;
     ...
     void dXX_YY(DARY* image_in, DARY* xx_image, float scale){
       ...
       DARY* image_tmp = new DARY(image_in->y(),image_in->x(),FLOAT1);
        DARY* image_tmp2 = new DARY(image_in->y(),image_in->x(),FLOAT1);
	...
	delete []coefs;delete image_tmp;delete image_tmp2;
     ...
     float smooth (int x, int y, DARY* image_in, float scale){       	
       DARY* image = new DARY(2*size+1,2*size+1,FLOAT1);
        DARY* image_tmp = new DARY(2*size+1,2*size+1,FLOAT1);
...
float smooth(int x, int y, DARY* image_in, float scalex, float scaley){
        ...
        int sizex=(int)rint(GAUSS_CUTOFF*scalex);
	int sizey=(int)rint(GAUSS_CUTOFF*scaley);
	...
	*/
    ...
    void dX (DARY* image_in,  DARY* image_out, float scalex, float scaley){		
      DARY* image_tmp = new DARY(image_out->y(),image_out->x(),FLOAT1);
	float* coefs = new float[17];
