...
{
   double sum;
   sum = a[0][0]*a[0][0]; sum += a[0][1]*a[0][1];
   sum += a[1][0]*a[1][0]; sum += a[1][1]*a[1][1];
   return ::sqrt(sum); 
}
...
void Matrix2::inv_i(void)
{
   double det;
   /* swap diagonal */
   det = a[0][0]; a[0][0] = a[1][1]; a[1][1] = det;
   /* calc determinant */
...
{
   a[0][0] = ::sqrt(a[0][0]); a[0][1] = ::sqrt(a[0][1]);
   a[1][0] = ::sqrt(a[1][0]); a[1][1] = ::sqrt(a[1][1]);
}
                     ...
                     double &cv, double &sv) const
{
   ...
   if (ga==0.0)
   {
      ...
      switch (pmax)
      {
      case 1: tsign = sign(cv) * sign(cu) * sign(f); break;
      case 2: tsign = sign(sv) * sign(cu) * sign(g); break;
      case 3: tsign = sign(sv) * sign(su) * sign(h); break;
      }
