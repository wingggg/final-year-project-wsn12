            ...
            int nin, const mxArray *in[])
{
  ...
  if(mxGetClassID(in[IN_DELTA]) != mxUINT8_CLASS) {
    mexErrMsgTxt("DELTA must be of class uint8") ;
  }
    ...
    while( true ) {
      ...
      */
      if(good && 
        ...
        idx_t nrindex, nvisited ;
        val_t nrvalue = 0 ;
        
        /* rindex = ROOT(index) might change as we merge trees,
           so we need to update it after each merge */
        ...
        /*
          ...
          b - I(ROOT(INDEX)) == I(ROOT(NRINDEX)). In this case index
              is extending an extremal region with the same value. In
              this case ROOT(NRINDEX) will NOT be an extremal region
              of I. So ROOT(INDEX) can be safely addedd as children of
              ROOT(NRINDEX), which often reduces the tree's height.
        ...
        */
        ...
        if( rindex != nrindex ) {
          ...
          if(nrvalue == value) {
            /* Tie. Often ROOT(index) is the shortest tree and becomes
               child */
